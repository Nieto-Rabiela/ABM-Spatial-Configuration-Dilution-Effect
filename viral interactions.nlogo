...
globals [ProbContagio listaExclusion]
...
to coinfeccion
  ...
  set listaExclusion [["A"] ["B"]]  ;exclusion [["A" "B"] ["B" "A"]]   coinfeccion [["A"] ["B"]]

  create-vectores 50 ;number of health vectors
  ...
  create-vectores 50 ; number of vectors with A
  [setxy random-xcor random-ycor
    set size 0.5
    set Life-Span 15 + random 50
    set color red
  ...
  create-vectores 50 ; number of vectors with B
  [setxy random-xcor random-ycor
    set size 0.5
    set Life-Span 15 + random 50
    set color green
  ...
  create-hospederos 150 ;number of hosts
  [setxy random-xcor random-ycor
    set size 1
    set color violet
    set es-vector False
...
to-report mePuedeDar [virus]
  ...
  let j 0
  let meDa True
  if member? virus enfermedades ; not to be reinfected
      [set meDa False]
  while [i < length enfermedades and meDa] ; compares whether the diseases carried by the agent are on its list of susceptibilities
  [
    set j 0
    let enfermedad  item i  enfermedades ; take the disease i
    while [j < length listaExclusion and meDa] ;check if the disease i is on their exclusion list
    [
...
if any? turtles-here with [ not es-vector and tick-infeccion <  8] ; it can get the virus until day 8
...
to Hospedero-Infecta-Vector-ant14
  if    any? turtles-here with [ es-vector and tick-infeccion < 8 or tick-infeccion > 14] ; it can not get the virus between 8 and 14 day
...
;antigenicidad cruzada a los 7 días----------------------------

to ant7
  clear-all
  reset-ticks
  set-default-shape vectores "bug"
  set listaExclusion [["A"] ["B"]]
...
to go-ant7
  ...
  ask vectores with [member? "A" (enfermedades) = True or member? "B" (enfermedades) = True ]
 ...
 ask hospederos with [member? "A" (enfermedades) = True or member? "B" (enfermedades) = True ]
  [set tick-infeccion tick-infeccion + 1
  ]
...
;antigenicidad efectiva entre el día 7 y 14----------------------------
...
to go-ant7-14
  ...
  [forward 1
    rt 50
    set Life-Span Life-Span - 1
        vector-infecta-hospedero ;host can not have vector special antigenicity
    set es-vector True
    reproduce-vector
    Vector-Muere
      ]
...
307
Vectors with A
count vectores with [enfermedades = (list \"A\")]+\ncount vectores with [enfermedades = (list \"A\" \"Nada\")]+\ncount vectores with [enfermedades = (list \"Nada\" \"A\")]+ \ncount vectores with [enfermedades = (list  \"A\")]
0
1
11
...
373
Vectors with B
count vectores with [enfermedades = (list \"B\")] +\ncount vectores with [enfermedades = (list \"Nada\" \"B\")]+\ncount vectores with [enfermedades = (list \"B\" \"Nada\")]+\ncount vectores with [enfermedades = (list \"B\")]
0
1
11
...
427
Vectors with AB
count vectores with [enfermedades = (list \"A\" \"Nada\" \"B\")]+\ncount vectores with [enfermedades = (list \"A\" \"B\" \"Nada\")]+\ncount vectores with [enfermedades = (list \"B\"\"Nada\" \"A\")]+\ncount vectores with [enfermedades = (list \"B\" \"A\" \"Nada\")]+\ncount vectores with [enfermedades = (list \"Nada\" \"B\" \"A\")]+\ncount vectores with [enfermedades = (list \"Nada\" \"A\" \"B\")]
0
1
11
...
106
Hosts with A
count hospederos with [enfermedades = (list \"A\")]+\ncount hospederos with [enfermedades = (list \"A\" \"Nada\")]+ \ncount hospederos with [enfermedades = (list \"Nada\" \"A\")]+\ncount hospederos with [enfermedades = (list \"A\")]
0
1
11
...
153
Hosts with B
count hospederos with [enfermedades = (list \"B\")]+\ncount hospederos with [enfermedades = (list \"Nada\" \"B\")]+\ncount hospederos with [enfermedades = (list \"B\" \"Nada\")]+\ncount hospederos with [enfermedades = (list \"B\")]
0
1
11
...
204
Hosts with AB
count hospederos with [enfermedades = (list \"A\" \"Nada\" \"B\")]+\ncount hospederos with [enfermedades = (list \"A\" \"B\" \"Nada\")]+\ncount hospederos with [enfermedades = (list \"B\"\"Nada\" \"A\")]+\ncount hospederos with [enfermedades = (list \"B\" \"A\" \"Nada\")]+\ncount hospederos with [enfermedades = (list \"Nada\" \"B\" \"A\")]+\ncount hospederos with [enfermedades = (list \"Nada\" \"A\" \"B\")]
0
1
11
